...
// Answer: For parallel execution I have used threads, so each thread runs specific operation, and dosen't waits other thread and operation to be done.
			//the gSum variable is a global one, so both of the threads using it parallel, where it makes no sense as each thread updating same variable.
			//so I have used mutex for this as a solution. 
